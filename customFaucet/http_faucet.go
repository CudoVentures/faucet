...
import (
	...
	"github.com/tendermint/faucet/customCoin"
...
type TransferRequest struct {
	...
	// Fees the request to be executed with.
...
func (f Faucet) faucetHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, coin := range coins {
		...
		if err := f.Transfer(r.Context(), req.AccountAddress, coin.amount, coin.denom, req.Fees); err != nil {
...
func (f Faucet) coinsToTransfer(req TransferRequest) ([]coin, error) {
	...
	for _, c := range req.Coins {
		amount, denom, err := customCoin.Parse(c)
		if err != nil {
			return nil, err
		}
