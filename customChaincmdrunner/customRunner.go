...
type Runner struct {
	chainCmd                      customChaincmd.ChainCmd
...
func New(ctx context.Context, chainCmd customChaincmd.ChainCmd, options ...Option) (Runner, error) {
	runner := Runner{
		chainCmd: chainCmd,
	...
	if chainCmd.IsAutoChainIDDetectionEnabled() {
		...
		runner.chainCmd = runner.chainCmd.Copy(customChaincmd.WithChainID(status.ChainID))
	}
...
func (r Runner) Cmd() customChaincmd.ChainCmd {
	return r.chainCmd
}
